...
- 通过 Android 官方 `DocumentsProvider` 将每个 OpenList 实例暴露为独立系统文件根。
- 支持系统文件应用中的浏览、读取及在后端能力允许时的新建、写入、重命名、移动、复制和删除。
- 随机读写由 App 私有 `no-backup` 事务草稿承接；提交采用暂存、备份、提升、校验和清理流程，绝不以 `Overwrite=true` 直接覆盖远端文件。
- 保存失败会保留 24 小时草稿并进入既有任务中心，供用户手动重试、恢复副本或删除；不会自动重新上传失败草稿。
- 不申请广域存储权限；第三方应用仅可通过 Android 系统授予的 URI 访问。
...
1. 先在 OpenList Android Client 中添加实例并完成登录或游客访问。
2. 在 Android 系统文件应用，或第三方应用的“打开”“另存为”“选择位置”流程中，选择 `OpenList · <实例名>` 根目录。
3. 系统界面会按当前会话、目录权限和存储驱动能力显示可用操作；不支持安全写入语义的操作会被隐藏或明确拒绝。
4. 登录失效后 root 仍会保留，但写入能力会撤回；重新进入 App 登录后即可刷新系统文件入口。
5. 如果关闭第三方应用时远端保存失败，请在 App 的“任务 → 失败”中处理“系统保存”任务，可手动重试、恢复副本或删除草稿。

系统文件功能没有独立的 App 页面，也不会把远端文件批量同步到本地。随机写入草稿只保存在 App 私有 no-backup 目录；草稿保存失败后保留 24 小时，且不会由后台自动重新上传。
...
- OpenList 离线下载任务。
- OpenList 复制、移动和转存任务。
- Android 系统文件写入产生的失败保存与恢复任务。

任务页面支持状态摘要、任务分组、失败项筛选、取消任务和清理任务记录。系统保存失败项还提供重试保存、恢复副本和删除草稿；OpenList 远程任务状态通过接口轮询更新。
...
- 二维码生成和扫描。
- 持续运行的全局后台任务轮询服务。
- 所有第三方编辑器的 close-time 保存错误提示保证（`onRelease` 没有可靠错误返回通道）。
- 对不提供安全重命名/移动能力的 OpenList 驱动进行危险写入回退。
