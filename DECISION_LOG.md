...
- **DEC-608（S6 静态审计发现，方案 A）**：PRD §14.1 第 10 项"Token 失效回登录页"自 v0.2 起从未真正实现——`SessionManager.invalidate()` 只删除本地 session 行，从不导航；只有管理台流程有自己的 `SESSION_EXPIRED` 跳转，主流程（文件/预览/上传等）401 后只显示内联错误条，`v0.2_ACCEPTANCE_REPORT.md` 曾误判此项"Accepted"。用户选择方案 A：在 v1.0 补做全局修复。实现：新增 `app/navigation/SessionExpiryViewModel`（复用 `AuthRepository.observeSession` 的 Room Flow 反应性，与管理台既有机制同源），在 `OpenListNavHost` 顶层按当前路由的 `instanceId` 订阅，检测到"曾有 session→变为 null"的边沿即导航回 `Routes.login(instanceId)`（`popUpTo(0)`），不影响管理台原有的独立跳转逻辑（两者并存无冲突）。3 个单测覆盖边沿检测逻辑；真机冒烟测试确认无回归/无崩溃。真实"已登录态在前台被后端 401 判定"的端到端复现因需真实 401 触发条件（改真实管理员密码风险过高，或不安全的运行时 SQLite 直接改库）未在本轮完成，记入 `v1.0_IMPLEMENTATION_LOG.md` S6 章节与 `KNOWN_ISSUES.md`。