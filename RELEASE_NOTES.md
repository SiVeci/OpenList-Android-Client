...
对比基线：v1.3.0 发布提交 `bc25d6c3d06f94fdde551a3c1821f8f40e681970`。

本版将 OpenList 实例接入 Android 官方系统文件生态，并为第三方应用写入增加可恢复的安全提交链路；不申请广域存储权限，也不改变既有 App 主导航和业务入口。

### 主要更新

- **系统文件入口**：通过 `DocumentsProvider` / SAF 将每个已配置 OpenList 实例提供为独立系统文件根；实例改名时 root ID 保持稳定，删除实例时仅清理对应 root 和本地映射。
- **浏览与文件操作**：支持目录浏览、元数据查询、创建文件和目录、重命名、删除，以及同实例内移动和复制；可用操作按会话、目录权限和后端能力动态开放，跨实例操作在远端副作用前拒绝。
- **随机读写**：通过 Proxy FD 支持顺序读取、seek、随机读取及 `w` / `wt` / `rw` / `rwt` / `wa` 写入模式；Range 不可信时只在空间允许的情况下使用 App 私有临时物化文件。
- **安全提交**：所有系统写入先落到 App 私有 `no-backup` 草稿；新建和覆盖采用“暂存 → 校验 → 备份原文件 → 提升 → 再校验 → 清理”的补偿式流程，所有上传均禁止 `Overwrite=true`。
- **失败恢复**：保存失败、结果待确认和清理中状态接入既有任务中心与通知；失败草稿保留 24 小时，支持用户手动重试保存、恢复副本或删除，后台不会自动重新上传失败草稿。
- **认证与安全**：会话失效时保留系统 root 并撤回写能力；`documentId` 使用不透明 UUID，签名下载地址不会携带不受信任的 OpenList 认证头，日志和通知不记录 Token 或完整签名 URL。

### 数据与内部调整

- Room Schema 从 10 升级至 11，新增系统文档稳定映射和写事务日志，并提供手写 `MIGRATION_10_11` 与迁移测试。
- 新增系统文档 Domain/Repository、Range HTTP 客户端、空间预留、路径锁、强提交、补偿恢复、TTL 清理和 WorkManager 调度链路。
- 统一任务模型新增“系统保存”来源；保存失败通知可直接进入对应实例的任务中心失败分组。
- 应用版本更新为 `versionCode = 11`、`versionName = "1.4.0"`；最低 Android 版本和 target SDK 保持不变。

### 验证与升级

- 全量 Debug 单元测试通过；API 29、35、36 上的 Room 迁移、Provider 和 Proxy FD 自动化通过。
- 隔离 OpenList 测试目录已完成真实创建、强提交、随机读取和删除验证；AOSP DocumentsUI 已完成 Provider 可发现性验证。
- Schema 10→11 自动迁移测试通过，但 v1.3.0 APK 到 v1.4.0 APK 的真实原地安装升级未执行。

### 已知边界

- Files by Google、WPS/Office 和另一随机读写第三方应用的人工实操按用户明确指示豁免，均为未执行，不应表述为通过。
- `ProxyFileDescriptorCallback.onRelease` 没有可靠的错误返回通道；关闭阶段提交失败时，以任务中心和通知中的恢复项为准。
- 多步远端替换是可补偿的逻辑事务，不宣称所有 OpenList 存储驱动提供原子交换；缺少安全重命名/移动语义的驱动不会回退到危险覆盖。
- 设备永久离线后立即卸载会丢失本地 no-backup 草稿和恢复日志，无法承诺远端临时物绝对零残留。
- 当前验收产物是未签名 Release APK；正式分发前仍需使用受控 keystore 签名。完整结论见 [v1.4_ACCEPTANCE_REPORT.md](v1.4_ACCEPTANCE_REPORT.md)。
